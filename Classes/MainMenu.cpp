...
bool MainMenu::init()
{
 ...
 //   this->addChild(label, 1);
	...
	auto item1 = MenuItemLabel::create(Label::createWithTTF("Play\n", "fonts/comic.ttf", 40), CC_CALLBACK_1(MainMenu::PlayMenu, this));
	auto item2 = MenuItemLabel::create(Label::createWithTTF("Background\n", "fonts/comic.ttf", 40), CC_CALLBACK_1(MainMenu::About, this));
	auto item3 = MenuItemLabel::create(Label::createWithTTF("Exit\n", "fonts/comic.ttf", 40), CC_CALLBACK_1(MainMenu::Exit, this));
	...
	// create menu, it's an autorelease object
	auto menu = Menu::create(item1, item2, item3, NULL);
	menu->setPosition(Director::getInstance()->getVisibleSize().width / 4 - 10, Director::getInstance()->getVisibleSize().height / 2);
	menu->alignItemsVerticallyWithPadding(FONT_SIZE / 2 + 5);
	this->addChild(menu, 2);
